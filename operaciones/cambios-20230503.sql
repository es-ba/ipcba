...
FOR vcaltpinf IN
  SELECT c.periodo, c.calculo, c.producto, c.division
         ...
         , EXP(AVG(LN(CASE WHEN c.AntiguedadIncluido>0 AND c.PromObs>0 AND c.ImpObs not in ('R','RA') AND ii.informante is not null and c.periodo >= ii.periodonegativo THEN c.PromObs ELSE NULL END))) as promImputadosInactivos
         , SUM(CASE WHEN c.AntiguedadIncluido>0 AND c.PromObs>0 AND c.ImpObs not in ('R','RA') AND ii.informante is not null and c.periodo >= ii.periodonegativo THEN 1 ELSE NULL END) as cantImputadosInactivos
    FROM CalObs c
    INNER JOIN cvp.calculos ca ON c.periodo=ca.periodo AND c.calculo=ca.calculo --PK verificada

    LEFT JOIN (SELECT p.producto, fi.informante, fi.formulario, i.periodonegativo
                FROM productos p
