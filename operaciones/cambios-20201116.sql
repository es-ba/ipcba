set search_path = cvp;
alter table reltar add column archivo_cache text;
alter table his.reltar add column archivo_cache text;

CREATE OR REPLACE FUNCTION cvp.hisc_reltar_trg()
    RETURNS trigger
    LANGUAGE 'plpgsql'
...
DECLARE
      ...
      IF v_operacion='I' THEN
				...
				INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_text_1,pk_number_2,pk_text_3,change_value,new_text)
                     VALUES ('cvp','RelTar','archivo_cache','I',new.periodo||'|'||new.panel||'|'||new.tarea,new.periodo,new.panel,new.tarea,'I:'||comun.a_texto(new.archivo_cache),new.archivo_cache);
      END IF;
      IF v_operacion='U' THEN
            ...
            END IF;  
			IF new.archivo_cache IS DISTINCT FROM old.archivo_cache THEN
                INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_text_1,pk_number_2,pk_text_3,change_value,old_text,new_text)
                     VALUES ('cvp','RelTar','archivo_cache','U',new.periodo||'|'||new.panel||'|'||new.tarea,new.periodo,new.panel,new.tarea,old.archivo_cache||'->'||new.archivo_cache
                     ,old.archivo_cache,new.archivo_cache);
            END IF;    			
      END IF;
      IF v_operacion='D' THEN
				...
				INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_text_1,pk_number_2,pk_text_3,change_value,old_text)
                     VALUES ('cvp','RelTar','archivo_cache','D',old.periodo||'|'||old.panel||'|'||old.tarea,old.periodo,old.panel,old.tarea,'D:'||comun.a_texto(old.archivo_cache),old.archivo_cache);
      END IF;
