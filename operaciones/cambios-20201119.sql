...
DECLARE    
  ...
  vPromPriImpAnt double precision;
  vrec record;
...
FOR vrec IN
  ...
  LOOP
    SELECT CASE WHEN vDescarteDefinitivoSegImp THEN t.CantRealesIncluidos ELSE t.CantIncluidos END, t.umbraldescarte, t.ImpDiv, t.PromPriImpAct, t.PromPriImpAnt
      INTO vCantIncluidosTipo, vumbraldescartetipo, vImpDiv, vPromPriImpAct, vPromPriImpAnt
      FROM CalDiv t
    ...
    IF (vrec.promObs is null OR vCantIncluidosTipo <vumbraldescartetipo OR vImpDiv='BII') AND vrec.promobsant>0 THEN
          UPDATE CalObs c 
            SET PromObs=CASE WHEN vPromPriImpAct is not null and vPromPriImpAnt is not null THEN vrec.promobsant*vPromPriImpAct/vPromPriImpAnt 
                             WHEN g0.Indice=0 THEN 0 ELSE vrec.promobsant*g.IndicePrel/g0.Indice END,
                 ImpObs=CASE WHEN vPromPriImpAct is not null and vPromPriImpAnt is not null THEN 'IODD' 
                             WHEN vImpDiv='BII' THEN vImpDiv||'-'||g.ImpGru WHEN g0.Indice=0 THEN 'A0' ELSE COALESCE(g.ImpGru,'AGV') END
            FROM  CalGru g ,
                  CalGru g0  
            WHERE  c.periodo=pPeriodo AND c.calculo=pCalculo
