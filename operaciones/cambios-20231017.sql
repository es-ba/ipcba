SET search_path = cvp;
alter table parametros add column planificacion_url text;
...
IF v_operacion='I' THEN 
    ...
    INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_bool_1,change_value,new_text)
         VALUES ('cvp','parametros','planificacion_url','I',new.unicoregistro,new.unicoregistro,'I:'||comun.a_texto(new.planificacion_url),new.planificacion_url);
END IF;
IF v_operacion='U' THEN    
    ...
    END IF;    
    IF new.planificacion_url IS DISTINCT FROM old.planificacion_url THEN
        INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_bool_1,change_value,old_text,new_text)
             VALUES ('cvp','parametros','planificacion_url','U',new.unicoregistro,new.unicoregistro,comun.A_TEXTO(old.planificacion_url)||'->'||comun.a_texto(new.planificacion_url),old.planificacion_url,new.planificacion_url);
    END IF;    
END IF;
IF v_operacion='D' THEN  
    ...
    INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_bool_1,change_value,old_text)
         VALUES ('cvp','parametros','planificacion_url','D',old.unicoregistro,old.unicoregistro,'D:'||comun.a_texto(old.planificacion_url),old.planificacion_url);
END IF;
