...
------------------------------------------------------------------------------------------------
SET search_path = cvp;
alter table parametros add column if not exists periodo_ccc character varying(11);
...
IF v_operacion='D' THEN
    ...
    INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_bool_1,change_value,old_text)
         VALUES ('cvp','parametros','periodo_ccc','D',old.unicoregistro,old.unicoregistro,'D:'||comun.a_texto(old.periodo_ccc),old.periodo_ccc);
END IF;
