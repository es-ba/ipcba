...
AS $BODY$
      ...
      IF v_operacion='I' THEN
        ...
        INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_text_1,pk_number_2,pk_number_3,pk_number_4,pk_number_5,change_value,new_text)
             VALUES ('cvp','relpantarinf','recuperos','I',new.periodo||'|'||new.informante||'|'||new.visita||'|'||new.panel||'|'||new.tarea,new.periodo,new.informante,new.visita,new.panel,new.tarea,'I:'||comun.a_texto(new.codobservaciones),new.codobservaciones);
      END IF;
      IF v_operacion='U' THEN          
        ...
        IF new.recuperos IS DISTINCT FROM old.recuperos THEN
            INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_text_1,pk_number_2,pk_number_3,pk_number_4,pk_number_5,change_value,old_text,new_text)
                 VALUES ('cvp','relpantarinf','recuperos','U',new.periodo||'|'||new.informante||'|'||new.visita,new.periodo,new.informante,new.visita,comun.A_TEXTO(old.recuperos)||'->'||comun.a_texto(new.recuperos),old.recuperos,new.recuperos);
        END IF;    
      END IF;
