...
SELECT pperiodo, pcalculo, p.producto, coalesce (ghr.responsable,gpr.responsable) as responsable, 'N' as revisado
...
--Ajustar responsables, si cambiaron
UPDATE cvp.calprodresp cp SET responsable = g.responsable
  FROM
  (SELECT hijos.grupo, hijos.grupopadre, coalesce(hijos.responsable, padres.responsable) responsable
  FROM cvp.grupos hijos 
  LEFT JOIN cvp.grupos padres on hijos.agrupacion = padres.agrupacion and hijos.grupopadre = padres.grupo
  WHERE hijos.agrupacion = 'F' and hijos.esproducto ='S') g
WHERE cp.periodo = pPeriodo and cp.calculo = pCalculo and cp.producto = g.grupo  
  and cp.responsable is distinct from g.responsable; 
