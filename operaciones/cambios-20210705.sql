SET search_path = cvp;
ALTER TABLE informantes ADD COLUMN "cluster" integer;
ALTER TABLE pantar ADD COLUMN modalidad text;
...
AS $BODY$
      ...
      IF v_operacion='I' THEN
                ...
                INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_number_1,change_value,new_text)
                     VALUES ('cvp','informantes','estado','I',new.informante,new.informante,'I:'||comun.a_texto(new.estado),new.estado);
                INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_number_1,change_value,new_number)
      ...
      IF v_operacion='U' THEN
            ...
            IF new.estado IS DISTINCT FROM old.estado THEN
                INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_number_1,change_value,old_text,new_text)
                     VALUES ('cvp','informantes','estado','U',new.informante,new.informante,comun.A_TEXTO(old.estado)||'->'||comun.a_texto(new.estado),old.estado,new.estado);
            END IF;			
      ...
      IF v_operacion='D' THEN
               ...
               INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_number_1,change_value,old_text)
                ...
                INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_number_1,change_value,old_text)
                     VALUES ('cvp','informantes','estado','D',old.informante,old.informante,'D:'||comun.a_texto(old.estado),old.estado);
                INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_number_1,change_value,old_number)
...
CREATE OR REPLACE FUNCTION hisc_pantar_trg() RETURNS trigger
    ...
    AS $$
      ...
      IF v_operacion='I' THEN
                ...
                INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_number_1,pk_number_2,change_value,new_text)
                     VALUES ('cvp','pantar','modalidad','I',new.panel||'|'||new.tarea,new.panel,new.tarea,'I:'||comun.a_texto(new.modalidad),new.modalidad);
      END IF;
      IF v_operacion='U' THEN
            ...
            IF new.modalidad IS DISTINCT FROM old.modalidad THEN
                INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_number_1,pk_number_2,change_value,old_text,new_text)
                     VALUES ('cvp','pantar','modalidad','U',new.panel||'|'||new.tarea,new.panel,new.tarea,comun.A_TEXTO(old.modalidad)||'->'||comun.a_texto(new.modalidad),old.modalidad,new.modalidad);
            END IF;    
      END IF;
      IF v_operacion='D' THEN
                ...
                INSERT INTO his.his_campos_cvp (esquema,tabla,campo,operacion,concated_pk,pk_number_1,pk_number_2,change_value,old_text)
                     VALUES ('cvp','pantar','modalidad','D',old.panel||'|'||old.tarea,old.panel,old.tarea,'D:'||comun.a_texto(old.modadlidad),old.modalidad);
      END IF;
