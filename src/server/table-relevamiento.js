    ...
    return be.tableDefAdapt({
        ...
        sql:{
            ...
            from: `(select rv.periodo, panel, tarea, rp.fechasalida, rv.informante, rv.visita, i.nombreinformante, i.direccion, i.contacto, i.telcontacto, i.web, i.email,
                    ...
                    nullif(count(distinct pre.formulario),0) as cantformulariostotales,
                    nullif(count(distinct case when pre.tipoprecio is not null then pre.formulario else null end),0) as cantformularioscompletos,  
                    nullif(count(distinct pre.formulario) - count(distinct case when pre.tipoprecio is not null then pre.formulario else null end),0) as cantformulariosfaltantes,
                    nullif(sum(case when pre.periodo is not null then 1 else 0 end),0) as cantpreciostotales,
                    nullif(sum(case when pre.tipoprecio is not null then 1 else 0 end),0) as cantprecioscompletos,
                    nullif(sum(case when pre.periodo is not null then 1 else 0 end) - sum(case when pre.tipoprecio is not null then 1 else 0 end),0) as cantpreciosfaltantes
                from (select * from personal per where per.username = ${db.quoteLiteral(context.user.usu_usu)}) per
                    ...
                    left join relpre pre on rv.periodo = pre.periodo and rv.informante = pre.informante and rv.visita = pre.visita and rv.formulario = pre.formulario
