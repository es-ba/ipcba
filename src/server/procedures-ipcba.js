...
function controlar_parametros(parametros_recibidos_en_la_funcion, definicion_de_parametros) {
  "use strict";
  for (var nombre_definido in definicion_de_parametros) {
    if (definicion_de_parametros.hasOwnProperty(nombre_definido)) {
  ...
  for (var nombre_recibido in parametros_recibidos_en_la_funcion) {
    if (parametros_recibidos_en_la_funcion.hasOwnProperty(nombre_recibido)) {
      if (!definicion_de_parametros.hasOwnProperty(nombre_recibido)) {
...
const handleCuadroH1 = async (client, params, cuadroInfo) => {
  ...
  const { rows } = await client.query(
    `SELECT * from ccc_cuadro_matriz_hogar('Listado de Valorización de la Canasta', $1, $2, $3, $4, $5, $6, $7)`,
    [params.periodo_desde, params.periodo_hasta, cuadroInfo.agrupacion, cuadro, cuadroInfo.hogares, params.separador_decimal, cuadroInfo.tipo_hogar]
  ).fetchAll();
...
const CuadroHandlers = {
  ...
  'ccc_cuadro_matriz_perfil': handleCuadroMP,
};
...
>>>>>>> master
  ...
  },
  {
    ...
    coreFunction: async function (context, params) {
      ...
      const filnamePartitio_0 = fileNamePartition[0];
      const filnamePartitio_1 = fileNamePartition[1].replace(/:/g, '-');
      const nombreCarpeta = './local-rescate';
      const filePath = `${nombreCarpeta}/${filnamePartitio_0}_${filnamePartitio_1}_${context.username || 'unlogged'}.txt`;
      try {
        await fs.mkdir(nombreCarpeta, { recursive: true });
        await fs.appendFile(filePath, `localStorage.setItem("${localStorageItemKey}", ${JSON.stringify(localStorageItem)})\n\n`, 'utf8');
        return 'ok';
      } catch (err) {
        console.log('ERROR', err);
        throw err;
