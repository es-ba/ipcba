...
FROM (SELECT p.periodo, p.informante, i.nombreinformante, i.tipoinformante, p.producto, prod.nombreproducto, p.visita, p.observacion, 
      ...
      left JOIN cvp.reltar t ON p.periodo = t.periodo AND v.panel = t.panel AND v.tarea = t.tarea
      left JOIN (SELECT periodo, cvp.moverperiodos(periodo, -5) as perreferencia
              FROM cvp.periodos 
              WHERE ingresando = 'S' OR periodo = (SELECT MAX(periodo) FROM cvp.periodos WHERE ingresando = 'N')) per ON p.periodo = per.periodo
      left JOIN cvp.relpre pant ON p.informante =pant.informante AND p.producto = pant.producto AND p.visita = pant.visita AND p.observacion = pant.observacion
      ...
      left JOIN cvp.productos prod ON p.producto = prod.producto
