...
FROM
  (SELECT * FROM cvp.caldiv WHERE  division = '0' and calculo = 0) o
  LEFT JOIN cvp.periodos r on o.periodo = r.periodo  
  LEFT JOIN (SELECT * FROM cvp.caldiv WHERE division = 'S' and calculo >= 0) s on o.periodo = s.periodo and o.calculo = s.calculo and o.producto = s.producto 
  LEFT JOIN (SELECT * FROM cvp.caldiv WHERE division = 'S' and calculo >= 0) s0 on s0.periodo = r.periodoanterior and s0.calculo = s.calculo and s0.producto = s.producto and s0.division = s.division 
  LEFT JOIN (SELECT * FROM cvp.caldiv WHERE division = 'T' and calculo >= 0) t on s.periodo = t.periodo and s.calculo = t.calculo and s.producto = t.producto 
  LEFT JOIN (SELECT * FROM cvp.caldiv WHERE division = 'T' and calculo >= 0) t0 on t0.periodo = r.periodoanterior and t.calculo = t0.calculo and t.producto = t0.producto and t.division = t0.division 
